        ...
        <div class="app-header-wrapper d-flex flex-stack w-100">
          ...
          <div id="kt_app_header_page_title_wrapper">
            ...
            <div data-kt-swapper="true" data-kt-swapper-mode="{default: 'prepend', lg: 'prepend'}" data-kt-swapper-parent="{default: '#kt_app_content_container', lg: '#kt_app_header_page_title_wrapper'}" class="page-title d-flex flex-column justify-content-center me-3 mb-6 mb-lg-0">
              <!--begin::Title-->
              <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center me-3 my-0">@if (isset($edit)) Edit @else Add @endif Initiation Guru</h1>
                ...
                <div class="card">
                  ...
                  <div class="card-body py-4">
                    ...
                    @endif
                      ...
                      @if (isset($edit))
                          <form method="post" action="{{ route('initiative-guru.update', $edit->id) }}" enctype="multipart/form-data">
                          @method('PUT')
                      @else
                          <form method="post" action="{{ route('initiative-guru.store') }}" enctype="multipart/form-data">
                      @endif
                      @csrf
                        <div class="row">
                            <div class="col-md-4 col-sm-4 col-lg-4 mb-4">
                                <div class="form-group">
                                    <label for="name">Devotee</label>
                                    <select class="form-control" name="name" id="name" required>
                                      <option value="">Select One</option>
                                      @if($devotees->count() != NULL)
                                        @foreach($devotees as $devotee)
                                          <option value="{{$devotee->id}}" @if (isset($edit)) {{ old('name', $edit->name == $devotee->id ? 'selected' : '' ?? '') }} @endif
