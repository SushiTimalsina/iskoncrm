        ...
        <div class="app-header-wrapper d-flex flex-stack w-100">
          ...
          <div id="kt_app_header_page_title_wrapper">
            ...
            <div data-kt-swapper="true" data-kt-swapper-mode="{default: 'prepend', lg: 'prepend'}" data-kt-swapper-parent="{default: '#kt_app_content_container', lg: '#kt_app_header_page_title_wrapper'}" class="page-title d-flex flex-column justify-content-center me-3 mb-6 mb-lg-0">
              <!--begin::Title-->
              <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center me-3 my-0">Initiation Guru Management</h1>
                  ...
                  <div class="card-header border-0 pt-6">
                    <div class="card-toolbar">
                      <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                        <a href="{{route('initiative-guru.create')}}" class="btn btn-primary">
                        <i class="ki-outline ki-plus fs-2"></i>Add Initiation Guru</a>
                        @if (Route::currentRouteName() == 'initiativegurutrash')
                        <a href="{{route('initiative-guru.index')}}" type="button" class="btn btn-primary ms-2">
                        <i class="ki-outline ki-file fs-2"></i>All Lists</a>
                        @else
                        <a href="{{route('initiativegurutrash')}}" type="button" class="btn btn-primary ms-2">
                  ...
                  <div class="card-body py-4">
                    ...
                    <div class="table-responsive">
                        ...
                        <tbody>
                          ...
                          <tr>
                            <td>{{$i}}</td>
                            <td><a href="{{ route('initiative-guru.show', $list->id)}}">
                                @if($list->name != NULL){{Crypt::decrypt($list->getdevotee->firstname)}}@endif
                                @if($list->name != NULL){{Crypt::decrypt($list->getdevotee->middlename)}}@endif
                                @if($list->name != NULL){{Crypt::decrypt($list->getdevotee->surname)}}@endif
                              </a> <a href="{{ route('devotees.show', $list->name)}}"><i class="ki-outline ki-exit-right-corner"></i></a></td>
                            <td>
                              {{Helper::getinitiatedevotees($list->id)->count()}}
                            </td>
                            <td>
                            ...
                            <td class="text-end">
                              ...
                              <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4" data-kt-menu="true">
                                ...
                                <div class="menu-item px-3">
                                  <a href="{{ route('initiativegururestore', $list->id)}}" class="btn btn-light d-block"><i class="ki-outline ki-arrows-loop fs-2"></i> Restore</a>
                                </div>
                                <div class="menu-item px-3">
                                  <form action="{{ route('initiative-guru.destroy', $list->id)}}" method="post">
                                    @csrf
                                    @method('DELETE')
                                ...
                                @else
                                  @can('initiative-list')
                                  <div class="menu-item px-3">
                                    <a href="{{ route('initiative-guru.show', $list->id)}}" class="btn btn-light d-block"><i class="ki-outline ki-eye fs-2"></i> View</a>
                                  </div>
                                  @endcan
                                  @can('initiative-edit')
                                  <div class="menu-item px-3">
                                    <a href="{{ route('initiative-guru.edit', $list->id)}}" class="btn btn-light d-block"><i class="ki-outline ki-pencil fs-2"></i> Edit</a>
                                  </div>
                                  @endcan
                                  @can('initiative-delete')
                                  <div class="menu-item px-3">
                                    <a href="{{ route('initiativegurumovetotrash', $list->id)}}" class="btn btn-light d-block"><i class="ki-outline ki-trash fs-2"></i> Trash</a>
                                  </div>
                                  @endcan
                                @endif
                              </div>
