        ...
        <div class="app-header-wrapper d-flex flex-stack w-100">
          ...
          <div id="kt_app_header_page_title_wrapper">
            ...
            <div data-kt-swapper="true" data-kt-swapper-mode="{default: 'prepend', lg: 'prepend'}" data-kt-swapper-parent="{default: '#kt_app_content_container', lg: '#kt_app_header_page_title_wrapper'}" class="page-title d-flex flex-column justify-content-center me-3 mb-6 mb-lg-0">
              <!--begin::Title-->
              <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center me-3 my-0">Branch Management</h1>
              <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                ...
                <li class="breadcrumb-item text-muted">
                  <a href="{{route('branch.index')}}" class="text-muted text-hover-primary">Branch Management</a>
                </li>
              </ul>
            </div>
          </div>
                  ...
                  <div class="card-header border-0 pt-6">
                    <div class="card-toolbar">
                      <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                        ...
                        <i class="ki-outline ki-plus fs-2"></i>Add Branch</button>
                        @if (Route::currentRouteName() == 'branchtrash')
                        <a href="{{route('branch.index')}}" type="button" class="btn btn-primary ms-2">
                        <i class="ki-outline ki-file fs-2"></i>All Lists</a>
                        @else
                        <a href="{{route('branchtrash')}}" type="button" class="btn btn-primary ms-2">
                                  ...
                                  <div class="row">
                                      <div class="col-md-12 col-sm-12 col-lg-12 mb-2">
                                          <div class="form-group">
                                              <label for="catname">Branch Name</label>
                    ...
                    <div class="table-responsive">
                        ...
                        <tbody>
                          ...
                          <tr>
                            <td>{{$i}}</td>
                            <td>{{$list->title}}</td>
                            <td>{{$list->getcreatedby->name}} - {{ \Carbon\Carbon::parse($list->created_at)->diffForhumans() }}</td>
                            <td>@if($list->updatedby != NULL){{$list->getcreatedby->name}} @else NA @endif - {{ \Carbon\Carbon::parse($list->updated_at)->diffForhumans() }}</td>
                            <td>
                            ...
                            <td class="text-end">
                              ...
                              <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4" data-kt-menu="true">
                                ...
                                <div class="menu-item px-3">
                                  <a href="{{ route('branchrestore', $list->id)}}" class="btn btn-light d-block"><i class="ki-outline ki-arrows-loop fs-2"></i> Restore</a>
                                </div>
                                <div class="menu-item px-3">
                                  <form action="{{ route('branch.destroy', $list->id)}}" method="post">
                                    @csrf
                                    @method('DELETE')
                                ...
                                @can('branch-edit')
                                <div class="menu-item px-3">
                                  <a href="{{ route('branch.edit', $list->id)}}" class="btn btn-light d-block"><i class="ki-outline ki-pencil fs-2"></i> Edit</a>
                                </div>
                                @endcan
                                @can('branch-delete')
                                <div class="menu-item px-3">
                                  <a href="{{ route('branchmovetotrash', $list->id)}}" class="btn btn-light d-block"><i class="ki-outline ki-trash fs-2"></i> Trash</a>
                                </div>
                                @endcan
                                @endif
                              </div>
                          ...
                          @if(count($list->subcategory))
                            @include('backend.branch.subCategoryList',['subcategories' => $list->subcategory])
                          @endif
                          <?php $i++; ?>
                          @endforeach
                        </tbody>
