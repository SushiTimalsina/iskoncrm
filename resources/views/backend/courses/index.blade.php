        ...
        <div class="app-header-wrapper d-flex flex-stack w-100">
          ...
          <div id="kt_app_header_page_title_wrapper">
            ...
            <div data-kt-swapper="true" data-kt-swapper-mode="{default: 'prepend', lg: 'prepend'}" data-kt-swapper-parent="{default: '#kt_app_content_container', lg: '#kt_app_header_page_title_wrapper'}" class="page-title d-flex flex-column justify-content-center me-3 mb-6 mb-lg-0">
              <!--begin::Title-->
              <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center me-3 my-0">Course Category</h1>
              <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                ...
                <li class="breadcrumb-item text-muted">
                  <a href="{{route('courses.index')}}" class="text-muted text-hover-primary">Course Category</a>
                </li>
              </ul>
            </div>
          </div>
                  ...
                  <div class="card-header border-0 pt-6">
                    <div class="card-toolbar">
                      <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                        ...
                        <i class="ki-outline ki-plus fs-2"></i>Add Course</button>
                        @if (Route::currentRouteName() == 'coursetrash')
                        <a href="{{route('courses.index')}}" type="button" class="btn btn-primary ms-2">
                        <i class="ki-outline ki-file fs-2"></i>All Lists</a>
                        @else
                        <a href="{{route('coursetrash')}}" type="button" class="btn btn-primary ms-2">
                        ...
                        <tbody>
                          ...
                          <tr>
                            ...
                            <td class="aligncenter">
                              @if($list->image != NULL)<a href="{{ route('coursephoto.show', ['imageName' => $list->image]) }}" data-fancybox="gallery"><img src="{{ route('coursephoto.show', ['imageName' => $list->image]) }}" alt="image" width="50" /></a>@endif
                            </td>
                            <td>{{$list->title}}</td>
                            <td>{{$list->getbranch->title}}</td>
                            <td>
                              @if(Helper::getfacilitatorbycourseid($list->id)->count() != NULL)
                              <ul>
                                @foreach(Helper::getfacilitatorbycourseid($list->id) as $facilitator)
                                  <li>{{Helper::getdevoteebyid($facilitator->devotee_id)->firstname}} {{Helper::getdevoteebyid($facilitator->devotee_id)->middlename}} {{Helper::getdevoteebyid($facilitator->devotee_id)->surname}}</li>
                                @endforeach
                              </ul>
                              @endif
                            </td>
                            ...
                            <td class="text-end">
                              ...
                              <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4" data-kt-menu="true">
                                ...
                                <div class="menu-item px-3">
                                  <a href="{{ route('courserestore', $list->id)}}" class="btn btn-light d-block"><i class="ki-outline ki-arrows-loop fs-2"></i> Restore</a>
                                </div>
                                <div class="menu-item px-3">
                                ...
                                @can('course-edit')
                                <div class="menu-item px-3">
                                  <a href="{{ route('courses.edit', $list->id)}}" class="btn btn-light d-block"><i class="ki-outline ki-pencil fs-2"></i> Edit</a>
                                </div>
                                @endcan
                                @can('course-delete')
                                <div class="menu-item px-3">
                                  <a href="{{ route('coursemovetotrash', $list->id)}}" class="btn btn-light d-block"><i class="ki-outline ki-trash fs-2"></i> Trash</a>
                                </div>
                                @endcan
                                @endif
                              </div>
                          ...
                          @if(count($list->subcategory))
                            @include('backend.courses.subCategoryList',['subcategories' => $list->subcategory])
                          @endif
                          @php $i++; @endphp
                          @endforeach
                        </tbody>
